...
**Datadog recommend to [enforce the Agent to send logs in HTTPS][2]**. Then refer to [set of proxy settings agent documentation][2] to configure HTTPS log forwarder and Proxy.
...
{{< tabs >}}
{{% tab "TCP" %}}
If you use a proxy for TCP transmission, configure the Datadog Agent to [enforce TCP transport][3] and send logs to your proxy through TCP using the following parameters in the `datadog.yaml` configuration file:
...
* `DD_LOGS_CONFIG_LOGS_DD_URL="<PROXY_ENDPOINT>:<PROXY_PORT>"`
...
[1]: /agent/proxy
[2]: /agent/logs/log_transport?tab=https#enforce-a-specific-transport
[3]: /agent/logs/log_transport?tab=tcp#enforce-a-specific-transport
